    ...
    <div className="modal-overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal-box">
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 18 }}>
          <h2 style={{ fontFamily: "'Literata',serif", fontSize: "clamp(18px,3vw,22px)", fontWeight: 700, color: "var(--p)", margin: 0 }}>
            Detalle del turno
          </h2>
          <button onClick={onClose} style={{ background: "transparent", border: "none", fontSize: 20, cursor: "pointer", color: "var(--secondary)" }}>✕</button>
        </div>

        <span style={{ display: "inline-block", padding: "4px 12px", borderRadius: 999, fontSize: 12, fontWeight: 700, background: ss.bg, color: ss.color, marginBottom: 16 }}>
          {turno.estado}
        </span>

        <div style={{ background: "var(--p-fixed)", borderRadius: 14, padding: "12px 16px", marginBottom: 14 }}>
          <div style={{ fontSize: 10, fontWeight: 700, textTransform: "uppercase", letterSpacing: ".07em", color: "var(--on-surf-v)", marginBottom: 4 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(160px,1fr))", gap: 10, marginBottom: 18 }}>
          {[
            ...
            <div key={label} style={{ background: "#fdfaf8", borderRadius: 10, padding: "10px 12px", border: "1px solid var(--outline-v)" }}>
              <div style={{ fontSize: 10, fontWeight: 700, textTransform: "uppercase", letterSpacing: ".06em", color: "var(--on-surf-v)", marginBottom: 3 }}>{label}</div>
        ...
        {turno.estado === "Reservado" && (
          <button
            ...
            style={{ width: "100%", padding: "12px 0", background: "var(--p)", color: "#fff", fontWeight: 700, borderRadius: 12, fontSize: 14, border: "none", cursor: "pointer", fontFamily: "inherit", marginBottom: 10 }}>
            🔄 Solicitar cambio de fecha
          </button>
        )}
        <button onClick={onClose}
          style={{ width: "100%", padding: "10px 0", background: "transparent", border: "2px solid var(--outline-v)", color: "var(--on-surf-v)", fontWeight: 600, borderRadius: 12, fontSize: 13, cursor: "pointer", fontFamily: "inherit" }}>
          Cerrar
        </button>
      </div>
    </div>
...
function CambioFechaModal({ turno, onClose, onConfirm }) {
  ...
  const buscar = async () => {
    ...
    setCargando(true); setError(""); setBuscado(false); setTurnoElegido(null);
    try {
      const p = {
    ...
    <div className="modal-overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal-box">
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
          <h2 style={{ fontFamily: "'Literata',serif", fontSize: "clamp(17px,3vw,20px)", fontWeight: 700, color: "var(--p)", margin: 0 }}>
            🔄 Cambio de fecha
          </h2>
          <button onClick={onClose} style={{ background: "transparent", border: "none", fontSize: 20, cursor: "pointer", color: "var(--secondary)" }}>✕</button>
        </div>

        <div style={{ background: "var(--p-fixed)", borderRadius: 12, padding: "10px 14px", marginBottom: 14, fontSize: 13, color: "var(--on-surf)" }}>
        ...
        <div style={{ marginBottom: 14 }}>
          <label style={{ display: "block", fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: ".06em", color: "var(--on-surf-v)", marginBottom: 6 }}>
            Nueva fecha desde
          </label>
          <div style={{ display: "flex", gap: 10 }}>
            <input
              ...
              style={{ flex: 1, padding: "10px 12px", borderRadius: 11, border: "1.5px solid var(--outline-v)", fontSize: 13, background: "#fff", color: "var(--on-surf)", fontFamily: "inherit" }}
            />
            <button onClick={buscar} disabled={!nuevaFecha || cargando}
              style={{ padding: "10px 16px", background: "var(--p)", color: "#fff", fontWeight: 700, borderRadius: 11, fontSize: 13, border: "none", cursor: !nuevaFecha || cargando ? "not-allowed" : "pointer", fontFamily: "inherit", opacity: !nuevaFecha || cargando ? 0.7 : 1, display: "flex", alignItems: "center", gap: 6 }}>
        ...
        {buscado && !cargando && turnosDisp.length === 0 && (
          <div style={{ textAlign: "center", padding: "20px 12px", color: "var(--secondary)", fontSize: 13, border: "1px dashed var(--outline-v)", borderRadius: 12, marginBottom: 14 }}>
            No hay turnos disponibles en esa fecha para la misma sede y servicio.
          </div>
        )}

        {turnosDisp.length > 0 && (
          <div style={{ maxHeight: 210, overflowY: "auto", display: "flex", flexDirection: "column", gap: 8, marginBottom: 14 }}>
            {turnosDisp.map(t => (
              <div key={t._id} onClick={() => setTurnoElegido(t)}
                style={{ padding: "10px 14px", borderRadius: 11, border: `2px solid ${turnoElegido?._id === t._id ? "var(--p)" : "var(--outline-v)"}`, background: turnoElegido?._id === t._id ? "var(--p-fixed)" : "#fff", cursor: "pointer", fontSize: 13, display: "flex", justifyContent: "space-between", alignItems: "center", transition: "all .15s" }}>
                <div>
                  <div style={{ fontWeight: 700, color: "var(--p)" }}>{fmtFecha(t.fechaHoraInicio)}</div>
        ...
        {turnoElegido && (
          <div style={{ background: "#e6f4eb", borderRadius: 10, padding: "10px 14px", marginBottom: 12, fontSize: 12, color: "#166534" }}>
        ...
        <button onClick={handleConfirmar} disabled={!turnoElegido || confirmando}
          style={{ width: "100%", padding: "12px 0", background: "var(--p)", color: "#fff", fontWeight: 700, borderRadius: 12, fontSize: 14, border: "none", cursor: !turnoElegido || confirmando ? "not-allowed" : "pointer", fontFamily: "inherit", opacity: !turnoElegido || confirmando ? 0.7 : 1, display: "flex", alignItems: "center", justifyContent: "center", gap: 8, marginBottom: 8 }}>
...
export default function PerfilPage() {
  ...
  return (
    <div style={{ maxWidth: "var(--page-max)", margin: "0 auto", padding: "clamp(20px,3vw,36px) var(--page-px) 56px" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "clamp(190px,22%,250px) 1fr", gap: 22, alignItems: "start" }} className="perfil-grid">

        {/* Sidebar */}
        <div className="wellness-card" style={{ borderRadius: 20, padding: 22 }}>
          <div style={{ width: 60, height: 60, borderRadius: 16, background: "var(--p-fixed)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 28, marginBottom: 12 }}>👤</div>
          <div style={{ fontSize: 15, fontWeight: 700, color: "var(--p)" }}>{usuario.nombreUsuario ?? "Mi cuenta"}</div>
          ...
          {menuItems.map(m => (
            <div key={m.id} onClick={() => setTab(m.id)}
              style={{ display: "flex", alignItems: "center", gap: 9, minHeight: 44, padding: "10px 12px", borderRadius: 11, fontSize: 13, fontWeight: tab === m.id ? 700 : 500, color: tab === m.id ? "var(--p)" : "var(--on-surf-v)", background: tab === m.id ? "var(--p-fixed)" : "transparent", cursor: "pointer", marginBottom: 3, transition: "all .15s" }}>
              <span>{m.icon}</span> {m.label}
            </div>
          ))}
          <div onClick={async () => { await logout(); router.push("/"); }}
            style={{ display: "flex", alignItems: "center", gap: 9, minHeight: 44, padding: "10px 12px", borderRadius: 11, fontSize: 13, fontWeight: 600, color: "#991b1b", cursor: "pointer", marginTop: 16 }}>
            🚪 Cerrar sesión
          </div>
        </div>
        ...
        <div>
          {tab === "turnos" && (
            <>
              <h2 style={{ fontFamily: "'Literata',serif", fontSize: "clamp(18px,3vw,22px)", fontWeight: 700, color: "var(--p)", marginBottom: 18 }}>
                Historial de turnos
              </h2>
              ...
              {historial.map(t => {
                ...
                return (
                  <div key={t._id} className="wellness-card"
                    ...
                    style={{ borderRadius: 14, padding: "14px 18px", display: "flex", alignItems: "center", gap: 12, marginBottom: 10, cursor: clickeable ? "pointer" : "default", flexWrap: "wrap" }}>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 8, flexShrink: 0, flexWrap: "wrap" }}>
                      <span style={{ padding: "3px 11px", borderRadius: 999, fontSize: 10, fontWeight: 700, background: ss.bg, color: ss.color }}>
                        {t.estado}
                      </span>
                      {clickeable && (
          ...
          {tab === "datos" && (
            <div className="wellness-card" style={{ borderRadius: 20, padding: 28 }}>
              <h2 style={{ fontFamily: "'Literata',serif", fontSize: "clamp(18px,3vw,22px)", fontWeight: 700, color: "var(--p)", marginBottom: 18 }}>Mis datos</h2>
      ...
      {turnoDetalle && (
        <TurnoDetalle
          ...
          onSolicitarCambio={t => { setTurnoDetalle(null); setTurnoCambio(t); }}
        />
      )}
      {turnoCambio && (
        <CambioFechaModal
