...
export default function RegistroMedicoPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      await registrarMedico({ nombreUsuario: form.nombreUsuario.trim(), password: form.password, nombre: form.nombre.trim(), matricula: form.matricula.trim() });
      setSuccess(true);
  ...
  const fields = [
    { name: "nombre",            label: "Nombre completo",     type: "text",     placeholder: "Ej: Dr. Juan Pérez",  autoComplete: "name" },
    { name: "matricula",         label: "Matrícula profesional", type: "text",   placeholder: "Ej: MN123456",         autoComplete: "off" },
    { name: "nombreUsuario",     label: "Nombre de usuario",   type: "text",     placeholder: "Mínimo 3 caracteres", autoComplete: "username" },
    { name: "password",          label: "Contraseña",          type: "password", placeholder: "••••••••",             autoComplete: "new-password", helper: PASSWORD_HELP },
    { name: "confirmarPassword", label: "Confirmar contraseña",type: "password", placeholder: "••••••••",             autoComplete: "new-password" },
  ];
    ...
    <div style={{ minHeight: "calc(100vh - 140px)", display: "flex", alignItems: "center", justifyContent: "center", padding: "48px 24px", background: "linear-gradient(135deg,#fdfaf8,#f9eef0 50%,#fdfaf8)" }}>
      <div style={{ width: "100%", maxWidth: 440 }}>
        <div style={{ textAlign: "center", marginBottom: 28 }}>
          ...
          <p style={{ fontSize: 13, color: "var(--secondary)" }}>Completá tus datos profesionales para empezar a atender pacientes.</p>
        </div>
