        ...
        <div className="wellness-card" style={{ borderRadius: 20, padding: 22 }}>
          ...
          {menuItems.map(m => (
            <div key={m.id} onClick={() => setTab(m.id)}
              style={{ display: "flex", alignItems: "center", gap: 9, padding: "10px 12px", borderRadius: 11, fontSize: 13, fontWeight: tab === m.id ? 700 : 500, color: tab === m.id ? "var(--p)" : "var(--on-surf-v)", background: tab === m.id ? "var(--p-fixed)" : "transparent", cursor: "pointer", marginBottom: 3, transition: "all .15s" }}>
              <span>{m.icon}</span> {m.label}
            </div>
          ))}
          <div onClick={async () => { await logout(); router.push("/"); }}
            style={{ display: "flex", alignItems: "center", gap: 9, padding: "10px 12px", borderRadius: 11, fontSize: 13, fontWeight: 600, color: "#991b1b", cursor: "pointer", marginTop: 16 }}>
            🚪 Cerrar sesión
          </div>
        </div>
