...
const S = {
  ...
  icon: { position: "absolute", left: 12, top: "50%", transform: "translateY(-50%)", fontSize: 15, color: "var(--on-surf-v)", pointerEvents: "none" },
  input: { width: "100%", padding: "11px 13px 11px 36px", borderRadius: 11, border: "1.5px solid var(--outline-v)", background: "#fff", color: "var(--on-surf)", fontSize: 14, fontFamily: "'Hanken Grotesk',system-ui,sans-serif", outline: "none", transition: "border .2s" },
};
    ...
    <div className="login-layout" style={{ minHeight: "calc(100vh - 140px)", display: "flex" }}>
      ...
      <div className="login-side-panel" style={{ width: "clamp(220px,42%,45%)", background: "var(--p)", padding: "clamp(28px,5vw,48px) clamp(24px,5vw,52px)", display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
        ...
        <div>
          ...
          <p style={{ fontSize: 14, color: "rgba(249,238,240,.72)", lineHeight: 1.6, marginBottom: 28 }}>Accedé a tu cuenta y gestioná tus turnos, historial y cobertura desde un solo lugar.</p>
          {["Turnos disponibles al instante","Historial médico completo","Cobertura de obra social integrada"].map((f) => (
            <div key={f} style={{ display: "flex", alignItems: "center", gap: 10, fontSize: 13, color: "rgba(249,238,240,.8)", marginBottom: 12 }}>
              <div style={{ width: 24, height: 24, borderRadius: 8, background: "rgba(255,255,255,.12)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>✓</div>
              {f}
            </div>
          ))}
        </div>
          ...
          <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 16 }} noValidate>
            ...
            <div>
              ...
              <div style={S.inputWrap}>
                ...
                <input name="nombreUsuario" type="text" autoComplete="username" placeholder="Tu nombre de usuario" value={form.nombreUsuario} onChange={handleChange} style={{ ...S.input, borderColor: errors.nombreUsuario ? "#991b1b" : "var(--outline-v)" }} />
              </div>
              {errors.nombreUsuario && <p style={{ fontSize: 11, color: "#991b1b", marginTop: 4 }}>{errors.nombreUsuario}</p>}
            </div>
            ...
            <div>
              ...
              <div style={S.inputWrap}>
                <span style={S.icon}>🔒</span>
                <input name="password" type="password" autoComplete="current-password" placeholder="••••••••" value={form.password} onChange={handleChange} style={{ ...S.input, borderColor: errors.password ? "#991b1b" : "var(--outline-v)" }} />
              </div>
