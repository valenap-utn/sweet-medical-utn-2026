            ...
            {items.map(t => {
              ...
              return (
                <div key={t._id} className="wellness-card" style={{ borderRadius:16, padding:"clamp(12px,2vw,17px) clamp(14px,2vw,20px)", display:"flex", alignItems:"center", gap:12, marginBottom:12, flexWrap:"wrap" }}>
                  ...
                  <button onClick={() => quitar(t._id)} aria-label="Quitar turno" style={{ width:44, height:44, borderRadius:11, border:"1px solid var(--outline-v)", background:"transparent", display:"flex", alignItems:"center", justifyContent:"center", cursor:"pointer", fontSize:16, color:"var(--secondary)", flexShrink:0 }}>
                    🗑
                  </button>
                </div>
              );
