...
const DATOS_FIJOS = {
  Basic: {
    ...
    noIncluye: ["Resonancias magnéticas", "Internaciones de larga estadía", "Tratamientos estéticos"],
    icono: "🩺",
  },
  Premium: {
    precio: 8900,
    ...
    noIncluye: ["Tratamientos estéticos", "Cirugías electivas no urgentes"],
    icono: "⭐",
  },
  Familiar: {
    precio: 14200,
    ...
    noIncluye: ["Tratamientos estéticos"],
    icono: "👨‍👩‍👧‍👦",
  },
};
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (activo) { setError(getApiErrorMessage(e, "")); setPlanes(FALLBACK_PLANES); }
    ...
    <div className="detalle-wrap">
      ...
      <section className="carnet-section">
        <div className={`carnet ${esPremium || esFamiliar ? "carnet-dark" : "carnet-light"}`}>
          ...
          <div className="carnet-front">
            ...
            <div className="carnet-plan-name">{datos.icono} {plan.nombre}</div>
            ...
            <div className="carnet-cob-list">
              {coberturas.slice(0, 6).map((c, i) => (
                <div key={i} className={`carnet-cob-item nivel-${claseNivel(c.nivel)}`}>
                  <span className="carnet-cob-icono">{iconoNivel(c.nivel)}</span>
      ...
      <section className="secciones-wrap">
        ...
        <div className="seccion-card seccion-plan">
          ...
          <div className="seccion-plan-header">
            ...
            <div className="seccion-plan-icon">{datos.icono}</div>
          </div>
        ...
        <div className="seccion-card">
          <div className="seccion-eyebrow incluye-eyebrow">✓ Qué incluye tu plan</div>
              ...
              coberturas
                ...
                .map((c, i) => (
                  <div key={i} className={`incluye-item nivel-${claseNivel(c.nivel)}`}>
                    <span className="incluye-icono">{iconoNivel(c.nivel)}</span>
                    <div>
        ...
        {/* Qué NO incluye */}
        <div className="seccion-card">
          <div className="seccion-eyebrow no-incluye-eyebrow">✕ Qué no incluye</div>
          <div className="no-incluye-list">
            ...
            {coberturas
              ...
              .map((c, i) => (
                <div key={`back-${i}`} className="no-incluye-item">
                  <span className="no-incluye-x">✕</span>
                  <span>{c.nombre}</span>
                </div>
              ))}
            {/* Items fijos */}
            ...
            {coberturas.filter((c) => normalizarNivel(c.nivel) === "NO_CUBIERTA").length === 0
              ...
              <div className="seccion-empty">Este plan cubre todas las prestaciones disponibles.</div>
            )}
          </div>
        </div>
      </section>
      ...
      <section className="comparativa-section">
        ...
        <p className="comparativa-subtitle">
          Tu plan actual (<strong>{plan.nombre}</strong>) aparece resaltado. Compará con las otras opciones disponibles.
        </p>
                    ...
                    .map((p) => (
                      <td key={p._id} className={p._id === plan._id ? "comp-td-active" : "comp-td"}>
                        <span className={`comp-badge nivel-${claseNivel(fila[p._id])}`}>
                          {iconoNivel(fila[p._id])} {textoNivel(fila[p._id])}
                        </span>
                      </td>
                    ))}
                </tr>
        ...
        <div className="comparativa-leyenda">
          <span className="leyenda-item"><span className="comp-badge nivel-total">✓ Total</span> Cubierto al 100%</span>
          <span className="leyenda-item"><span className="comp-badge nivel-partial">◑ Parcial</span> Cobertura parcial</span>
          <span className="leyenda-item"><span className="comp-badge nivel-none">✕ No cubierta</span> No incluido</span>
        </div>
      </section>
